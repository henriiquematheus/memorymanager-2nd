...
type MemoryManager struct {
	physicalMemory [][]interfaces.SubProcess
...
func NewMemoryManager(pageSize int, physicalMemorySize int) *MemoryManager {
	...
	return &MemoryManager{
		physicalMemory: make([][]interfaces.SubProcess, pages),
...
func (mm *MemoryManager) writeWithPaging(p *process.Process) {
	...
	spaces := int(math.Ceil(float64(len(subProcesses)) / float64(mm.pageSize)))
